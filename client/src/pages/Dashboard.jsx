import React, { useEffect, useState } from 'react'
import { useNavigate } from 'react-router-dom'

function Dashboard() {
  const [isVeryfied,setIsVerified] = useState(localStorage.getItem('2fa_verify'))
  const navigate = useNavigate()

  useEffect(()=>{
    console.log(isVeryfied,"isVeryfied");
    if(!isVeryfied){
      navigate('/')
    }
  },[isVeryfied])

  const handeleLogout= () => {
    localStorage.removeItem('token')
    localStorage.removeItem('2fa_verify')
    navigate('/')
  }


  return (
    <div className='flex justify-center mt-10'>
    <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
  <div href="#">
    <img className="rounded-t-lg" src="/docs/images/blog/image-1.jpg" alt="" />
  </div>
  <div className="p-5">
    <div href="#">
      <h2 className='text-4xl mb-3'>Dashboard</h2>
      <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
       Welcome to the 2fa check system using google authentication.
      </h5>
    </div>
    <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
      This the demo system for check two step authentication
    </p>
    <div
     onClick={handeleLogout}
      className="cursor-pointer inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
    >
      Logout
      <svg
        className="rtl:rotate-180 w-3.5 h-3.5 ms-2"
        aria-hidden="true"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 14 10"
      >
        <path
          stroke="currentColor"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={2}
          d="M1 5h12m0 0L9 1m4 4L9 9"
        />
      </svg>
    </div>
  </div>
</div>
</div>

  )
}

export default Dashboard